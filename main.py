...
def main(camera_id=0):
    ...
    lowcut = 0.1  # Corresponds to 6 breaths/min
    highcut = 0.5 # Corresponds to 30 breaths/min
    filter_order = 4
    ...
    # Main loop
    while True:
        current_bpm = 0.0 # Initialize BPM (Breaths Per Minute) for the current frame
        try:
            ...
            if results.multi_face_landmarks:
                ...
                for face_idx, face_landmarks in enumerate(results.multi_face_landmarks):
                    ...
                    if valid_roi_for_signal:
                        ...
                        cv2.putText(frame, f"RGB (Signal Src): {r_signal_source:.0f}, {g_signal_source:.0f}, {b_signal_source:.0f}", 
                                   (20, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                        ...
                        if len(rppg_buffer) > 10: 
                            ...
                            if raw_signal_for_graph.size > 0:
                                ...
                                if max_val - min_val > 1e-6: # Check for non-flat signal
                                    ...
                                    raw_graph_img.fill(0) # Clear graph if signal is flat
                            ...
                            if fs > 0 and (highcut * 2 < fs): 
                                ...
                                if low > 0 and high < 1 and low < high:
                                    ...
                                    if filtered_rppg.size > 0:
                                        ...
                                        if max_filt_val - min_filt_val > 1e-6: # Check for non-flat signal
                                            
                                            # --- START BPM (Breaths Per Minute) CALCULATION ---
                                            if fs > 0: 
                                                # Distance: peaks should be at least 0.8s apart for respiration (max ~75 breaths/min, well above our filter)
                                                # Our filter is 0.1-0.5 Hz (6-30 breaths/min). A 1-second distance (fs samples) should be fine.
                                                # Let's use a distance that corresponds to slightly less than the lowest expected breath rate.
                                                # e.g., if lowest is 6 breaths/min (0.1 Hz), period is 10s.
                                                # A more robust approach for peak detection in respiration:
                                                # distance = fs / (highcut * 1.5) # e.g. fs / (0.5 * 1.5) = fs / 0.75
                                                # Or simply a fixed time like 1.5-2 seconds for respiration.
                                                # 6 breaths/min = 1 breath per 10s. 30 breaths/min = 1 breath per 2s.
                                                # Peaks should be at least ~1.5 seconds apart.
                                                distance_samples = int(fs * 1.5) # Min distance between peaks in samples
                                                
                                                std_dev_filtered = np.std(filtered_rppg)
                                                prominence_value = 0.20 * std_dev_filtered if std_dev_filtered > 1e-6 else None # Adjusted prominence

                                                peaks, properties = signal.find_peaks(filtered_rppg, distance=distance_samples, prominence=prominence_value)

                                                if len(peaks) >= 2:
                                                    peak_intervals_samples = np.diff(peaks)
                                                    avg_interval_samples = np.mean(peak_intervals_samples)
                                                    
                                                    if avg_interval_samples > 0: 
                                                        avg_interval_seconds = avg_interval_samples / fs
                                                        if avg_interval_seconds > 0: 
                                                            calculated_bpm = 60.0 / avg_interval_seconds
                                                            current_bpm = np.clip(calculated_bpm, 5, 35) # Clip to 5-35 breaths/min
                                            # --- END BPM CALCULATION ---
                                            ...
                                            filtered_graph_img.fill(0)
                                            cv2.putText(filtered_graph_img, "Flat Signal", (10, graph_height // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                                else:
                                    filtered_graph_img.fill(0) 
                    ...
                    if 'filtered_graph_img' in locals():
                        ...
                        y_offset_for_filt_graph = 20 + h_raw + 10 
                        if y_offset_for_filt_graph + h_filt < frame.shape[0]: 
                             ...
                             bpm_text_display = f"Breaths/min: {current_bpm:.1f}" if current_bpm > 0 else "Breaths/min: N/A"
                             text_color_bpm = (0, 255, 255) if current_bpm > 0 else (128, 128, 128)
                             cv2.putText(frame, bpm_text_display, (frame.shape[1] - 20 - w_filt, y_offset_for_filt_graph + h_filt + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color_bpm, 1)
                ...
                # Clear graphs if no face is detected
                raw_graph_img.fill(0)
                filtered_graph_img.fill(0)
                cv2.putText(raw_graph_img, "No Face", (10, graph_height // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                cv2.putText(filtered_graph_img, "No Face", (10, graph_height // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                 # Display the cleared graphs
                h_raw, w_raw = raw_graph_img.shape[:2]
                frame[20:20+h_raw, frame.shape[1]-20-w_raw:frame.shape[1]-20] = raw_graph_img
                cv2.putText(frame, "Raw rPPG (Green Channel)", (frame.shape[1]-20-w_raw, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
                h_filt, w_filt = filtered_graph_img.shape[:2]
                ...
                if y_offset_for_filt_graph + h_filt < frame.shape[0]:
                    frame[y_offset_for_filt_graph : y_offset_for_filt_graph + h_filt, frame.shape[1]-20-w_filt : frame.shape[1]-20] = filtered_graph_img
                    cv2.putText(frame, "Filtered rPPG", (frame.shape[1]-20-w_filt, y_offset_for_filt_graph - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 1)
                    cv2.putText(frame, "Breaths/min: N/A", (frame.shape[1] - 20 - w_filt, y_offset_for_filt_graph + h_filt + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128,128,128), 1)
            ...
            current_bpm = 0.0 
            # break # Optionally break loop on any error, or try to continue
    ...
    if collected_raw_rppg_signal and len(collected_raw_rppg_signal) > 2 * filter_order + 1:
        ...
        if plot_fs > 0 and (highcut * 2 < plot_fs): 
        ...
        if filtered_plot_signal is not None:
            ...
            plt.plot(final_time_axis, detrended_plot_signal, label='Detrended rPPG Signal (Filter Failed)', color='red') 
